  ...
  useEffect(() => {
    const fetchCakes = async () => {
      try {
        ...
        const filterData = data.filter((e) => e.tag.includes(id));
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Header & Breadcrumb */}
        <div className="mb-4">
          <h1 className="text-2xl font-bold text-gray-900 mb-1">{id} Online</h1>
          <div className="text-sm text-gray-500 mb-2">
            Home <span className="mx-1">&gt;</span> <span className="text-gray-700 font-medium">{id}</span>
          </div>
        </div>
        {/* Tab & Filters */}
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2 mb-4">
          <div className="flex items-center gap-4">
            <div className="bg-white rounded-t-lg px-6 py-2 font-semibold text-gray-900 border-b-2 border-rose-400 shadow-sm">
              {id} <span className="text-gray-500 font-normal">({cakes.length} items)</span>
            </div>
          </div>
